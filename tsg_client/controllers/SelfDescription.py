...
class OfferedResource:
    ...
    def from_dict(obj: Any) -> "OfferedResource":
        if "ids:resourceEndpoint" in obj.keys():
            ...
            documentation = obj["ids:resourceEndpoint"][0].get(
        ...
        created = (
            str(obj.get("ids:created").get("@value"))
            if obj.get("ids:created")
            else None
        )
        title = (
            str(obj.get("ids:title")[0].get("@value")) if obj.get("ids:title") else None
        )
        description = (
            str(obj.get("ids:description")[0].get("@value"))
            if obj.get("ids:description")
            else None
        )
...
class SelfDescription:
    ...
    def from_dict(obj: Any) -> "SelfDescription":
        ...
        try:
            ...
            _endpoints = str(
                obj.get("ids:hasDefaultEndpoint").get("ids:accessURL").get("@id")
            )
            resource_catalogs = obj.get("ids:resourceCatalog", [])
            _catalogs = (
                [ResourceCatalog.from_dict(y) for y in resource_catalogs]
                if resource_catalogs
                else []
            )
